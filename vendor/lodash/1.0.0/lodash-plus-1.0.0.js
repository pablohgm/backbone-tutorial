...
_.isInvalid = function(argValue){
    if( _.isUndefined(argValue) || _.isNull(argValue) ){
        return true;
    }
    return false;
};

_.isInvalidOrEmpty = function(argValue){
    if( _.isUndefined(argValue) || _.isNull(argValue) || _.isEmpty(argValue) ){
        return true;
    }
    return false;
};

_.getValueOrDefault = function(argValue,argDefault){
    if( _.isInvalid(argValue) ){
        return argDefault;
    }
    return argValue;
};

_.format = function(argString){
    if(_.isInvalidOrEmpty(arguments) || _.isInvalidOrEmpty(argString) ){
        return "";
    }
    var tmpFormatValues = _(arguments).slice(1,arguments.length);
    tmpFormatValues = tmpFormatValues.__wrapped__;
    for( var tmpIndex=0;tmpIndex < tmpFormatValues.length;tmpIndex++ ){
        var tmpFormatValue = tmpFormatValues[tmpIndex];
        var tmpToReplace = "{" + tmpIndex + "}";
        argString = argString.replace(tmpToReplace,tmpFormatValue);
    }
    return argString;
};

_.getAsArray = function(argValue){
    if( _.isInvalid(argValue) ){
        return null;
    }
...
_.concatArrayAsString = function(argArray,argSeparator){
    if( _.isInvalidOrEmpty(argArray) ){
        return "";
    }
    var tmpSeparator = _.getValueOrDefault(argSeparator,", ");
    var tmpArrayString = "";
    for( var tmpIndex=0;tmpIndex < argArray.length;tmpIndex++ ){
        var tmpArrayValue = argArray[tmpIndex];
        tmpArrayString += tmpArrayValue;
        if( (tmpIndex + 1) < argArray.length ){
            tmpArrayString += tmpSeparator;
        }
    }
    return tmpArrayString;
};

_.concatArrayOfObjectsAsString = function(argArray,argValueField,argSeparator){
    if( _.isInvalidOrEmpty(argArray) || _.isInvalid(argValueField) ){
        return "";
    }
    var tmpSeparator = _.getValueOrDefault(argSeparator,", ");
    var tmpArrayString = "";
    for( var tmpIndex=0;tmpIndex < argArray.length;tmpIndex++ ){
        var tmpArrayValue = argArray[tmpIndex];
        tmpArrayString += tmpArrayValue[argValueField];
        if( (tmpIndex + 1) < argArray.length ){
            tmpArrayString += tmpSeparator;
        }
    }
...
_.executeCallback = function(argCallback,argCallbackContext,argArgs){
    if( _.isInvalid(argCallback) || _.isInvalid(argCallbackContext) ){
        return;
    }
    argCallback.call(argCallbackContext,argArgs);
}
